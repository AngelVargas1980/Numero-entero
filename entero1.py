...
class Desk:
    def __init__(self, window):
        ...
        #(frame, text = 'Sumar', command = self.sumar).grid(row = 3, columnspan = 2, sticky = W + E)
        #(frame, text = 'Restar', command = self.restar).grid(row = 4, columnspan = 3, sticky = W + E)
        #(frame, text = 'Multiplicar', command = self.multiplicar).grid(row = 5, columnspan = 4, sticky = W + E)
    ...
    def dividir(self):
        if self.validation():
            ...
            self.message['text'] = 'La division NO ES EXACTA: {}'.format(resultado)
        else:
            self.message['text'] = 'LA DIVISIÓN ES EXACTA'
